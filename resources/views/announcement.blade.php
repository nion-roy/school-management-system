...
<div class="container-fluid page-header py-5 mb-5 wow fadeIn" data-wow-delay="0.1s" style="background-image: linear-gradient(rgba(15, 66, 41, 0.6), rgba(15, 66, 41, 0.6)), url( {{ Storage::disk('public')->url('setting/'.$f_settings->banner) }})">
	<div class="container text-center py-5">
		<h1 class="display-3 text-white mb-4 animated slideInDown">{{ Str::words($announcement->title, 3) }}</h1>
			...
			<div class="row" id="left_artical">
				...
				<div class="col-12 mb-3">
					<div class="border">
						<div class="col-12 px-4 py-2 bg-success">
							<h5 class="m-0 fw-bold text-white">ঘোষনা</h5>
						</div>
					</div>
				...
				<div class="col-12">
					...
					<div class="border p-2">
						<ul class="navbar-nav">
							<li class="nav-item"><a class="nav-link py-1" href="{{ Storage::disk('public')->url('announcement/' . $announcement->pdf) }}"><i class="fas fa-download me-2"></i> {{ Str::words($announcement->title, 8, '...') }} <span class="float-end">তারিখ: {{ $announcement->created_at->format('d-M-Y') }}</span></a></li>
						</ul>
					</div>
