	...
	<div class="container text-center py-5">
		...
		<nav aria-label="breadcrumb animated slideInDown">
			<ol class="breadcrumb justify-content-center mb-0">
				...
				<li class="breadcrumb-item active" aria-current="page">News Detials</li>
			</ol>
		</nav>
	</div>
</div>
	...
	<div class="container">
		<div class="row">
			<div class="col-md-5 mx-auto mb-3">
				<img src="{{ Storage::disk('public')->url('news/' . $newses->image) }}" class="img-fluid rounded shadow" alt="{{ $newses->title }}">
			</div>
			<div class="col-12">
				<div class="mb-4">
					<h4 class="mt-4">{{ $newses->title }}</h4>
					<span class="me-3"> <strong><i class="fas fa-user text-success"></i> Posted by:</strong> {{ Auth::user()->name }}</span>
					<span> <strong><i class="far fa-clock text-success"></i> Published:</strong> {{ $newses->created_at->format('d M, Y') }}</span>
