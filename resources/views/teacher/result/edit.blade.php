	...
	<div class="col-sm-12">
		...
		<div class="card">
			...
			<div class="card-body">
				...
				<form action="{{ route('teacher.result.update', $result->id) }}" method="post">
					@csrf
					@method('PUT')
					...
					<div class="form-group row">
						...
						<div class="col-xl-9 col-sm-8">
							<select id="class_dropdown" name="class_id" class="form-control select2 w-100">
								...
								<option value="{{ $class->id }}" {{ $result->class_id == $class->id ? 'selected' : '' }}>{{ $class->batch }}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-xl-9 col-sm-8">
							<select name="student_id" class="form-control select2" id="roll_dropdown">
								...
								@foreach ($student as $roll)
								@if ($result->class_id == $roll->class_id)
								<option value="{{ $roll->id }}" {{ $result->student_id == $roll->id ? 'selected' : '' }}>{{ $roll->roll }} - {{ $roll->class->batch }}</option>
								@endif
								@endforeach
					...
					<div class="form-group row">
						...
						<div class="col-sm-1 text-center d-none d-sm-block">:</div>
						<div class="col-xl-9 col-sm-8">
							<input type="text" id="name" name="name" class="form-control" placeholder="Enter Student Name" value="{{ $result->student->name }}">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-xl-9 col-sm-8">
							<select id="subject_dropdown" name="subject_id" class="form-control select2 w-100">
								...
								<option value="{{ $subject->id }}" {{ $result->subject_id == $subject->id ? 'selected' : '' }}>{{ $subject->subject }}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-sm-1 text-center d-none d-sm-block">:</div>
						<div class="col-xl-9 col-sm-8">
							<input type="text" id="mark" name="mark" class="form-control" placeholder="Enter Total Mark" value="{{ $result->mark }}">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-xl-9 col-sm-8">
							<div class="custom-control custom-checkbox">
								<input class="custom-control-input custom-control-input-danger" type="checkbox" id="customCheckbox4" name="status" {{ $result->status == true ? 'checked' : '' }}>
