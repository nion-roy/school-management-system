	...
	<div class="col-sm-12">
		...
		<div class="card">
			...
			<div class="card-body">
				...
				<form action="{{ route('admin.student-result.update', $result->id) }}" method="post">
					@csrf
					@method('PUT')
					...
					<div class="form-group row align-items-center">
						...
						<div class="col-xl-9 col-sm-8">
							<select name="student_roll" class="form-control select2" id="roll_dropdown">
								...
								<option value="{{ $roll->id }}" {{ $result->student_roll == $roll->id ? 'selected' : '' }}>{{ $roll->roll }} - {{ $roll->class->batch }}</option>
								@endif
								@endforeach
					...
					<div class="form-group row align-items-center">
						...
						<div class="col-sm-1 text-center d-none d-sm-block">:</div>
						<div class="col-xl-9 col-sm-8">
							<input type="text" id="name" name="name" class="form-control" placeholder="Enter Student Name" value="{{ $result->user->name }}" disabled>
						</div>
					</div>
